 ...
 * Get the trim's ID.
 *
 * @return {string} The trim's ID
 *
 */
Trim.prototype.getTrimID = function() {
  if (this.trim.showBuild == 'Y') return this.trim.trimId
  else return 0
}
...
Trim.prototype.getImage = function() {
  return absoluteURL(this.trim.resptrimImage)
}
...
Trim.prototype.getMaxSeating = function() {
  var seating = this.trim.seating
  if (typeof seating === 'string') {
    var seats = underscoreGS._map(seating.split(','), function(count) {
      return parseInt(count.replace(/\s+/g, ''))
    })
